...
// Replace dynamic phone number with static one
const phoneNumber = "+1 (800) 123-4567"; // Replace with your desired phone number

// Replace email service with a simple function
const sendLandingPageFormEmail = async (formData, trustedFormData) => {
  try {
    // Implement your email sending logic here
    console.log('Form data:', formData);
    console.log('Trusted form data:', trustedFormData);
    return { success: true };
  } catch (error) {
...
export default function ChatInterface() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      try {
        const result = await sendLandingPageFormEmail(verificationForm, {
  ...
  const validateForm = () => {
    ...
    if (!capVal) {
      errors.captcha = "Please verify you're not a robot";
      isValid = false;
    }
