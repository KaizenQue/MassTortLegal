...
import ChatImg from "../assets/f1.webp"
...
export default function ChatInterface() {
  ...
  const validateForm = () => {
    ...
    if (!verificationForm.name.trim()) {
      ...
      isValid = false
    } else if (verificationForm.name.trim().length < 2) {
    ...
    if (!verificationForm.number.trim()) {
      ...
      errors.number = 'Please enter a valid US phone number'
      isValid = false
    }
    ...
    >
      ...
      <div className="relative z-10 flex flex-col h-full max-w-md mx-auto w-full">
        ...
        <div className="flex items-center gap-2 mb-4 bg-black/30 p-3 rounded-lg">
          ...
          <span className="text-white font-medium">Alexandra is online</span>
        </div>
          ...
          {showInitialMessage && (
            <div className="flex items-start gap-2">
              <div className="bg-gray-700/80 text-white py-2 px-3 rounded-lg rounded-tl-none max-w-[80%]">
                <div className="text-xs text-gray-300 mb-1">Alexandra</div>
                <p>Hi 👋</p>
              </div>
            </div>
          )}
